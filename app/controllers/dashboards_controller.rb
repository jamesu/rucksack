class DashboardsController < ApplicationController
  ...
  def show
    ...
    #@recent_activities = ApplicationLog.find(:all, :order => 'created_on DESC', :select => "created_by_id, rel_object_id, rel_object_type, object_name, page_id, action_id, date(created_on, '+#{Time.zone.utc_offset} seconds') AS sorted_on, created_on", :group => 'created_by_id, CASE page_id ISNULL WHEN 1 THEN rel_object_type || rel_object_id ELSE page_id END, sorted_on').group_by do |obj|
    
     @recent_activities = ApplicationLog.find(:all, :order => 'created_on DESC', :group => "created_by_id, date(created_on, '+#{Time.zone.utc_offset} seconds'), CASE page_id ISNULL WHEN 1 THEN rel_object_type || rel_object_id ELSE page_id END").group_by do |obj|
